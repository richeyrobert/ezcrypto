= EzCrypto2 - Easy to use Crypto for Ruby

EzCrypto2 is an easy to use wrapper around the poorly documented OpenSSL ruby library.

This is a complete and total plagiarization of Pelle's original EzCrypto gem. I only changed one single line to remove an OpenSSL deprecation issue that I got sick of seeing. 
...
Generate a key using a password and a salt. Use the keys encrypt method to encrypt a strings worth of data:
  ...
  @key=EzCrypto2::Key.with_password "password", "system salt"
...
These simple examples use one line each:
  ...
  @encrypted=EzCrypto2::Key.encrypt_with_password "password", @salt,"Top secret should not be revealed"
  
  EzCrypto2::Key.decrypt_with_password "password", @salt,@encrypted
...
If you already have a key from some other source, you simply have to call the constructor with the raw data:
  ...
  @key=EzCrypto2::Key.new @binarykey
...
EzCrypto2 is optimized for simple encryption and decryption of strings. There are encrypt/decrypt pairs for normal binary use as well as for Base64 encoded use.
...
EzCrypto2 and ActionCrypto is released under the MIT license.
